...
public class EditorialTasksCollectionResource
{
   private final TrcApplication trcCtx;

   public EditorialTasksCollectionResource(TrcApplication trcCtx)
   {
      this.trcCtx = trcCtx;
   }
   ...
   {
      // belongs on parent resource -- this should create items
      TrcAccount account = bean.get(TrcAccount.class);
   ...
   private Supplier<EntryReference<?>> makeEntryRefSupplier(EntryRepository<?> repo)
   {
      Iterator<?> iterator = repo.listAll();
      return () -> {
         Object entry = null;
         ...
         return entry != null ? resolvers.getReference(entry) : null;
      };
   }
